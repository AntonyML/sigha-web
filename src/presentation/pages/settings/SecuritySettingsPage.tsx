 ...
 *   1. Política de contraseñas     (funcional — endpoint /settings/security)
 *   2. 2FA obligatorio             (funcional — endpoint /settings/security)
 *   3. Sesiones                    (funcional: política | stub: sesiones activas)
 *   4. Intentos de login           (funcional: política | stub: historial)
 *
 * UX/UI copiada de GeneralSettingsPage / InterfaceSettingsPage:
 *   - lp-page, lp-header, lp-card, lp-table, lp-badge, lp-empty, lp-error
...
export default function SecuritySettingsPage() {
  ...
  // ── Estado de políticas (todas funcionales vía /settings/security JSONB) ─
  ...
  // ── Datos en vivo (stubs, no implementados aún) ─────────────────────────
  const [sessions] = useState<ActiveSession[]>([]);
  const [attempts] = useState<LoginAttemptRecord[]>([]);
  const [attemptsTotal] = useState(0);
    ...
    (async () => {
      try {
        ...
        setPasswordPolicy({
          ...
          passwordExpirationDays: 0, preventPasswordReuse: 0,
        });
  ...
  const handleSave2FA = async () => {
    ...
    setSaving2FA(true); setError(null); setMessage(null);
    try {
  ...
  const handleSaveSession = async () => {
    ...
    setSavingSession(true); setError(null); setMessage(null);
    try {
  ...
  const handleSaveAttempts = async () => {
    ...
    setSavingAttempts(true); setError(null); setMessage(null);
    try {
  ...
  return (
    <div className="lp-page">
      ...
      <div className="lp-header">
        ...
        <button type="button" className="lp-btn lp-btn--back" onClick={() => navigate('/settings')}>
          <ChevronLeft size={16} />
      ...
      {error && (
        <div className="lp-error">
          {error}
          <button className="lp-error__retry" onClick={() => setError(null)}>×</button>
        </div>
      )}
      {message && (
      ...
      {/* ──────────────────────────────────────────────────────────────────
          Tab 1: Política de contraseñas (funcional)
         ────────────────────────────────────────────────────────────────── */}
      {activeTab === 'password' && passwordPolicy && (
        <div className="lp-card" style={{ padding: '1.5rem' }}>
          <h3 className="lp-card-title" style={{ fontSize: '1.0625rem', fontWeight: 600, marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <div style={{ display: 'grid', gap: '1rem', maxWidth: '640px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', maxWidth: '480px' }}>
              <ToggleRow label="Mayúsculas requeridas" checked={passwordPolicy.requireUppercase} disabled={!hasEdit} onChange={v => setPasswordPolicy(prev => prev ? { ...prev, requireUppercase: v } : prev)} />
              <ToggleRow label="Minúsculas requeridas" checked={passwordPolicy.requireLowercase} disabled={!hasEdit} onChange={v => setPasswordPolicy(prev => prev ? { ...prev, requireLowercase: v } : prev)} />
              <ToggleRow label="Números requeridos"    checked={passwordPolicy.requireNumbers}    disabled={!hasEdit} onChange={v => setPasswordPolicy(prev => prev ? { ...prev, requireNumbers: v } : prev)} />
              <ToggleRow label="Caracteres especiales" checked={passwordPolicy.requireSpecialChars} disabled={!hasEdit} onChange={v => setPasswordPolicy(prev => prev ? { ...prev, requireSpecialChars: v } : prev)} />
            </div>
            ...
            <div style={{ marginTop: '0.5rem' }}>
              <button type="button" disabled={savingPassword || !hasEdit} onClick={handleSavePassword} className="lp-btn lp-btn--primary" style={{ background: '#dc2626', borderColor: '#dc2626' }}>
                {savingPassword ? <><RefreshCw size={16} className="animate-spin" /> Guardando…</> : <><Save size={16} /> Guardar política</>}
              </button>
              {!hasEdit && <p style={{ marginTop: '0.5rem', fontSize: '0.8125rem', color: '#94a3b8' }}>Solo permiso de visualización.</p>}
            </div>
          </div>
        </div>
      )}
      ...
      {/* ──────────────────────────────────────────────────────────────────
          Tab 2: 2FA obligatorio (funcional)
         ────────────────────────────────────────────────────────────────── */}
      {activeTab === 'twofactor' && twoFactorPolicy && (
        <div className="lp-card" style={{ padding: '1.5rem' }}>
          ...
          <div style={{ display: 'grid', gap: '1rem', maxWidth: '640px' }}>
            <ToggleRow label="Forzar 2FA para todos los usuarios" checked={twoFactorPolicy.enforceForAll} disabled={!hasEdit} onChange={v => setTwoFactorPolicy(prev => prev ? { ...prev, enforceForAll: v } : prev)} />

            <div>
              <label className="block font-medium mb-1">Roles obligados a 2FA (separados por coma)</label>
              <input
            ...
            <div style={{ marginTop: '0.5rem' }}>
              <button type="button" disabled={saving2FA || !hasEdit} onClick={handleSave2FA} className="lp-btn lp-btn--primary" style={{ background: '#dc2626', borderColor: '#dc2626' }}>
                {saving2FA ? <><RefreshCw size={16} className="animate-spin" /> Guardando…</> : <><Save size={16} /> Guardar política 2FA</>}
              </button>
            </div>
          </div>
        </div>
      ...
      {/* ──────────────────────────────────────────────────────────────────
          Tab 3: Sesiones (funcional: política | stub: sesiones activas)
         ────────────────────────────────────────────────────────────────── */}
          ...
          <div style={{ display: 'grid', gap: '1rem', maxWidth: '640px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
              <div>
                ...
                <input type="number" min={0} max={20} value={sessionPolicy.maxConcurrentSessions} disabled={!hasEdit} onChange={e => setSessionPolicy(prev => prev ? { ...prev, maxConcurrentSessions: parseInt(e.target.value) || 0 } : prev)} className="input" />
              ...
              <div>
                ...
                <input type="number" min={1} max={1440} value={sessionPolicy.sessionTimeoutMinutes} disabled={!hasEdit} onChange={e => setSessionPolicy(prev => prev ? { ...prev, sessionTimeoutMinutes: parseInt(e.target.value) || 30 } : prev)} className="input" />
              </div>
            </div>
            <div>
              <label className="block font-medium mb-1">Duración máxima de sesión (horas)</label>
              <input type="number" min={0} max={168} value={sessionPolicy.maxSessionDurationHours} disabled={!hasEdit} onChange={e => setSessionPolicy(prev => prev ? { ...prev, maxSessionDurationHours: parseInt(e.target.value) || 0 } : prev)} className="input" />
            ...
            <div style={{ marginTop: '0.5rem' }}>
              <button type="button" disabled={savingSession || !hasEdit} onClick={handleSaveSession} className="lp-btn lp-btn--primary" style={{ background: '#dc2626', borderColor: '#dc2626' }}>
                {savingSession ? <><RefreshCw size={16} className="animate-spin" /> Guardando…</> : <><Save size={16} /> Guardar política</>}
              </button>
            </div>
          </div>
        </div>
      ...
      {/* ──────────────────────────────────────────────────────────────────
          Tab 4: Intentos de login (funcional: política | stub: historial)
         ────────────────────────────────────────────────────────────────── */}
          ...
          <div style={{ display: 'grid', gap: '1rem', maxWidth: '640px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.75rem' }}>
              <div>
                ...
                <input type="number" min={1} max={20} value={loginAttemptsPolicy.maxAttempts} disabled={!hasEdit} onChange={e => setLoginAttemptsPolicy(prev => prev ? { ...prev, maxAttempts: parseInt(e.target.value) || 5 } : prev)} className="input" />
              </div>
              <div>
                <label className="block font-medium mb-1">Bloqueo (min)</label>
                <input type="number" min={1} max={1440} value={loginAttemptsPolicy.lockoutDurationMinutes} disabled={!hasEdit} onChange={e => setLoginAttemptsPolicy(prev => prev ? { ...prev, lockoutDurationMinutes: parseInt(e.target.value) || 15 } : prev)} className="input" />
              </div>
              <div>
                <label className="block font-medium mb-1">Ventana (min)</label>
                <input type="number" min={1} max={1440} value={loginAttemptsPolicy.attemptWindowMinutes} disabled={!hasEdit} onChange={e => setLoginAttemptsPolicy(prev => prev ? { ...prev, attemptWindowMinutes: parseInt(e.target.value) || 30 } : prev)} className="input" />
              </div>
            </div>
            <ToggleRow label="Reiniciar contador al iniciar sesión correctamente" checked={loginAttemptsPolicy.resetAttemptsOnSuccess} disabled={!hasEdit} onChange={v => setLoginAttemptsPolicy(prev => prev ? { ...prev, resetAttemptsOnSuccess: v } : prev)} />
            <div style={{ marginTop: '0.5rem' }}>
              <button type="button" disabled={savingAttempts || !hasEdit} onClick={handleSaveAttempts} className="lp-btn lp-btn--primary" style={{ background: '#dc2626', borderColor: '#dc2626' }}>
                {savingAttempts ? <><RefreshCw size={16} className="animate-spin" /> Guardando…</> : <><Save size={16} /> Guardar política</>}
              </button>
            </div>
          </div>
        </div>
      ...
      {activeTab === 'attempts' && (
        <div className="lp-card" style={{ overflow: 'hidden' }}>
          ...
          <div className="lp-table-wrap">
            <div className="lp-empty" style={{ padding: '4rem' }}>
              <AlertTriangle size={48} className="lp-empty__icon" />
              <p>Funcionalidad en desarrollo, requiere endpoint dedicado.</p>
            </div>
          </div>
        </div>
...
// ═══════════════════════════════════════════════════════════
//  Sub-componentes (fuera de SecuritySettingsPage)
// ═══════════════════════════════════════════════════════════
