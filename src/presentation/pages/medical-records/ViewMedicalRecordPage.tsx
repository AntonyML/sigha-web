...
import {
  FileText, ArrowLeft, Pencil, User, Stethoscope, ClipboardList, AlertCircle, Calendar, UserCheck
} from 'lucide-react'
import { medicalRecordService, type MedicalRecord } from '../../../services/medicalRecordService'
import '../../styles/lp.css'

const ORIGIN_AREA_LABELS: Record<string, string> = {
  nursing: 'Enfermería',
...
const getFullPatientName = (r: MedicalRecord): string => {
  const p = r.id_older_adult;
  if (!p) return '—';
  return [p.oa_name, p.oa_f_last_name, p.oa_s_last_name].filter(Boolean).join(' ') || '—';
};

const getStaffName = (r: MedicalRecord): string => {
  const s = r.id_staff;
  if (!s) return '—';
  return [s.u_name, s.u_f_last_name].filter(Boolean).join(' ') || '—';
};

const fmt = (v?: string | null) => v && v.length > 0 ? v : '—';
    ...
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '1.5rem 1.25rem 3rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
          ...
          <div>
            ...
            <p style={{ margin: '0.125rem 0 0', fontSize: '0.8125rem', color: '#64748b' }}>
              Creado el {fmtDateTime(record.create_at)}
            </p>
          </div>
        </div>
      ...
      <div style={{ background: 'linear-gradient(135deg, #1e40af 0%, #2563eb 100%)', borderRadius: '0.875rem', padding: '1.25rem 1.5rem', marginBottom: '1rem', color: '#fff', display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{ margin: 0, fontSize: '1.125rem', fontWeight: 700 }}>{getFullPatientName(record)}</p>
          {record.id_older_adult?.oa_identification && (
            <p style={{ margin: '0.125rem 0 0', fontSize: '0.8125rem', opacity: 0.85 }}>
              Cédula: {record.id_older_adult.oa_identification}
            </p>
          )}
        </div>
        <div style={{ textAlign: 'right' }}>
          ...
          <p style={{ margin: 0, fontSize: '1rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
            <Calendar size={14} /> {fmtDate(record.mr_record_date)}
          </p>
          <div style={{ marginTop: '0.375rem' }}>
            <span style={{ background: 'rgba(255,255,255,0.25)', borderRadius: '999px', padding: '0.2rem 0.75rem', fontSize: '0.75rem', fontWeight: 700 }}>
              {ORIGIN_AREA_LABELS[record.mr_origin_area] ?? record.mr_origin_area}
            </span>
          </div>
        </div>
      </div>
      ...
      <Section title="Médico / Responsable" icon={<UserCheck size={16} />}>
        <Field label="Nombre" value={getStaffName(record)} />
        {record.id_staff?.u_email && <Field label="Correo electrónico" value={record.id_staff.u_email} />}
        {record.mr_signed_by && <Field label="Firmado por" value={record.mr_signed_by} />}
        {record.id_appointment && <Field label="ID Cita especializada" value={String(record.id_appointment.id)} />}
      </Section>

      {/* Resumen clínico */}
      <Section title="Resumen clínico" icon={<ClipboardList size={16} />}>
        <Field label="Resumen" value={fmt(record.mr_summary)} wide />
      </Section>

      {/* Evaluación y plan */}
      <Section title="Evaluación y plan" icon={<Stethoscope size={16} />}>
        <Field label="Diagnóstico" value={fmt(record.mr_diagnosis)} wide />
        <Field label="Tratamiento" value={fmt(record.mr_treatment)} wide />
        <Field label="Observaciones" value={fmt(record.mr_observations)} wide />
      </Section>
