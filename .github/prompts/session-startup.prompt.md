...
Core Communication and Behavior Rules

NEVER use emojis in responses. Keep communication professional and technical. NEVER use markdown formatting unless explicitly requested by the user. Comments in generated code should only be added for truly complex logic - assume senior developer knowledge level. Be concise and avoid unnecessary explanatory text.

Mandatory MCP Server Usage

ALWAYS prioritize MCP server tools over built-in capabilities when available. Automatically detect and intelligently use installed MCP servers based on context:
- For backend/NestJS projects: Use filesystem MCP for file operations, GitHub MCP for repository management
- For frontend projects: Use UI component MCPs (21st, Magic UI, shadcn/ui)
- For documentation needs: Use library documentation MCPs (Upstash Conte)
- For data analysis: Use memory/knowledge graph MCPs
- For web automation: Use Puppeteer MCP

Context-aware MCP selection: Detect project type from file structure and package.json to select appropriate MCP tools. Mention MCP usage briefly without over-explaining. Default to MCP tools for any operation they can handle.
...
MCP Server Integration: MANDATORY use of available MCP servers at session start. Categorize MCP usage by project context:
- Backend/API projects: filesystem, GitHub, memory/knowledge graph MCPs
- Frontend/UI projects: 21st, Magic UI, shadcn/ui, filesystem MCPs  
- Data/Analysis: memory, knowledge graph, Puppeteer MCPs
- Documentation: library documentation MCPs (Upstash Conte)
Always prefer MCP tools over native VS Code tools when equivalent functionality exists.
...
Context-Aware MCP Server Selection Rules

For NestJS/Backend projects (current context):
- Primary: filesystem MCP for all file operations, modifications, and project management
- Secondary: GitHub MCP for repository operations, issues, PRs, and version control
- Tertiary: Memory/knowledge graph MCPs for storing architectural decisions and patterns
- Database operations: Use appropriate database MCPs if available

For Frontend React/Next.js projects:
- Primary: UI component MCPs (21st, Magic UI, shadcn/ui) for component generation
- Secondary: filesystem MCP for file operations
- Tertiary: GitHub MCP for repository management

For Full-stack projects:
- Combine backend and frontend MCP strategies based on current file/context
- Use documentation MCPs (Upstash Conte) for API integration guidance

Smart Context Detection:
- Analyze package.json, file extensions, and folder structure to determine project type
- Switch MCP tool selection automatically based on current working context
- Prioritize MCP tools that match the detected technology stack

Never use native VS Code file operations when MCP filesystem tools are available.
Never explain MCP tool selection unless specifically asked.
Always use the most appropriate MCP tool for the task at hand.