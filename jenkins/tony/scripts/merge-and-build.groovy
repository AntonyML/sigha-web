def mergeDevToMaster(sourceRepo, credentialsId) {
    withCredentials([usernamePassword(credentialsId: credentialsId, usernameVariable: 'GIT_USER', passwordVariable: 'GIT_PASS')]) {
        bat '''
            ...
            set "URL_WITH_CREDS=https://%GIT_USER%:!PASS_ENCODED!@git.ucr.ac.cr/proyecto_analisis/frontend_proton_react_hogar_de_ancianos.git"
            
            REM Limpiar cambios locales y archivos generados
            git clean -fd test-results jmeter-results build dist 2>nul
            git reset --hard HEAD
            git checkout dev
            git pull origin dev
