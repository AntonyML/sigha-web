param(
    ...
    [int]$FtpPort = 21,
    [string]$RemotePath = "/",
    [string]$SourceBuild = ".\build",
    [string]$ProductionPath = "C:\ProyectoAnalisis\frontend\www"
)

Write-Host "========================================"
Write-Host "FTP DEPLOYMENT - REACT BUILD - ZIP OPTIMIZED"
Write-Host "========================================"
Write-Host ""
...
try {
    ...
    $remoteFileUri = "$ftpUri$RemotePath/build.zip"
    ...
    $buildPath = Join-Path $ProductionPath "build"
    if (-not (Test-Path $buildPath)) {
        New-Item -ItemType Directory -Path $buildPath -Force | Out-Null
    }
    
    # Extraer ZIP local en la carpeta build
    Write-Host "Extracting ZIP file from: $zipPath"
    Add-Type -AssemblyName System.IO.Compression.FileSystem
    
    try {
        [System.IO.Compression.ZipFile]::ExtractToDirectory($zipPath, $buildPath)
        Write-Host "ZIP extracted successfully to $buildPath"
    } catch {
    ...
    if (Test-Path $zipPath) {
        Remove-Item $zipPath -Force
        Write-Host "Cleanup: Removed local build.zip"
    }
